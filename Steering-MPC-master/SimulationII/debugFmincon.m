...
eta_ref=zeros(3,Hp,length(t)); %Inertial frame Refernce Trajectories
...
%    Compute nonlinear cornering force of the tires, front and back.
    [Fc_f,dFy_f]=Pacejka_F_dF(alph_ac(1,i),p_Bf,p_Cf,p_Df,p_Ef);
    [Fc_r,dFy_r]=Pacejka_F_dF(alph_ac(2,i),p_Br,p_Cr,p_Dr,p_Er);
    F0_f = Fc_f - dFy_f*alph_ac(1,i);
    F0_r = Fc_r - dFy_r*alph_ac(2,i);
    % create LTV model of the nonlinear system
    bike_sys=[-2*(dFy_f+dFy_r)/(m*xdot),-2*(dFy_f*a-dFy_r*b)/(m*xdot)-xdot;...
        -2*(dFy_f*a-dFy_r*b)/(J*xdot),-2*(dFy_f*a^2+dFy_r*b^2)/(J*xdot)];
    bike_in=2*[dFy_f/m,(F0_f+F0_r)/m;(dFy_f*a)/J,(F0_f*a-F0_r*b)/J];
    bike_out=[1,0;0,1];
    bike_ff=0;
