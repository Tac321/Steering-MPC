...
%FINDTHETA returns theta, the index of the point on the centerline which is
%the     The DISCRETE POINT, LIEK THE DISCRETE POINTS OF THE TRACK, LIKE
%FOLLOWING POINTS.
...
TrackX = TrackCenter(1,:);  % The entire track x-coords
...
if last_closestIdx > N_Track - 20 || last_closestIdx <= 5
    ...
    searchRegion(i) = N_Track+searchRegion(i);  % lOCaL SEARCH OF THE DISCRETIZED POINT.
    searchRegion(k) = searchRegion(k)-N_Track;
end

% compute squared distance to the current location
TrackXsmall = TrackX(searchRegion); % searchRegion is the neighbor hood discrete index about the current thetaV we were previously closest to.
TrackYsmall = TrackY(searchRegion);  % Here is the x-y coord of the actual center track.
distanceX = TrackXsmall - posX*ones(1, N_searchRegion );  % Mesureing the closest to the current position, of the local search region.
...
closestIdx = minIndex;    % if this is the closest index of the closest cone.

% Look at my notes, remember what this meant/was 
