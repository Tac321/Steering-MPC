...
% Note the first terms in the Xmpc and Umpc are the actual x0 and u0 for
% this current iteration. We calculating the Umpc(1) s.t. u0 + Umpc(1) = u0
% for the next iteration. SCD: Awesome OMG!
stage(1).x0 = x0;
stage(1).u0 = u0;
...
for i = 1:MPC_vars.N
    ...
    stage(i).Qk = costScale*generateH(traj,MPC_vars,ModelParams,Xk,i);  % [10 x 10]
    ...
    [stage(i).Ck, stage(i).ug, stage(i).lg] = getInequalityConstraints(borders(max(i-1,1),:),MPC_vars,ModelParams);  % First two border values are identical, we'll crop  off the very first one in quadProgInterface
    % bounds
...
% quadratic rate input cost 
stage(i).Rk = costScale*1*diag([MPC_vars.rdD; MPC_vars.rdDelta; MPC_vars.rdVtheta]);
% linear state(-input) cost
stage(i).fk = costScale*generatef(traj,MPC_vars,ModelParams,Xk,i);
% linearized track constraints
[stage(i).Ck, stage(i).ug, stage(i).lg] = getInequalityConstraints(borders(i-1,:),MPC_vars,ModelParams); % Stuff the last term in there here.
% bounds
[stage(i).lb, stage(i).ub] = getBounds(MPC_vars,ModelParams);
...
elseif MPC_vars.interface == "quadprog"
    ...
    [X,U,dU,info] = QuadProgInterface(stage,MPC_vars,ModelParams); % This output isn't X_inertial frame, but W which is the step from initial position we must take to get to the minimum!
else
...
%     end
    ...
    Qk = 1*blkdiag(Qtilde,diag([MPC_vars.rD,MPC_vars.rDelta,MPC_vars.rVtheta]));
    % scale cost
...
function f = generatef(pathinfo,MPC_vars,ModelParams,Xk,i)
  ...
  %  X = quadprog([1 2 3;4 5 6;7 8 9],zeros(3,1),[],[],[],[],[],[],[1;2;3])
    fx=1*e'*Q*grad_e - 1*Xk'*grad_e'*Q*grad_e; %  Oriignally both terms were multilpied by 
...
function [Ak,Bk,gk] = getEqualityConstraints(Xk,Uk,MPC_vars,ModelParams) 
    ...
    [Ad, Bd, gd]=DiscretizedLinearizedModel(Xk,Uk,ModelParams,MPC_vars.Ts); % Formed from good initial guesses into Newton method. Performed on xMPC_i, uMPC_i
    ...
    gk = [MPC_vars.Tx*gd;zeros(nu,1)];  % [10 x 1]
...
function [Ck, ug, lg] = getInequalityConstraints(border,MPC_vars,ModelParams)
    ...
    % MPC predicted values based on thetaV projected forward in MPC time.
    % Left  border (x,y)
    x1=border(1);
    y1=border(2);
    % Right border (x,y)
    ...
    % Lucky this works at psiVirtual = 45 and 225 == -135 degrees
